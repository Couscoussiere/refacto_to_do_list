import { fetchProjectDates, validateDueDateAgainstProject, fetchProjectStatus, } from '../service/projectServiceClient.js';
...
export const createTaskController = (params) => {
    ...
    const createTask = async (req, res) => {
        try {
            ...
            if (body.dueDate !== undefined && dueDate === undefined) {
                res.status(400).json({
                    message: 'dueDate must be an ISO date string (YYYY-MM-DD) or null',
                });
                return;
            }
            if (dueDate) {
    ...
    const updateTask = async (req, res) => {
        try {
            ...
            const projectStatus = await fetchProjectStatus(existingTask?.projectId ?? -1, getAuthToken(req));
            if (projectStatus == 'DONE') {
                res.status(422).json({
                    message: 'Le projet est terminer, aucune tache peut etre modifier',
                });
                return;
            }
