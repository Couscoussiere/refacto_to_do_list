...
describe("PUT /tasks/:id", () => {
    ...
    it("passage en DONE → publie TaskCompleted", async () => {
        ...
        expect(rabbit.publishEvent).toHaveBeenCalledWith(expect.objectContaining({ type: "TaskCompleted" }));
    });
    it("passage en IN_PROGRESS → publie TaskStarted", async () => {
        await request(app).put(`/tasks/${taskId}`).send({ status: "IN_PROGRESS" });
        expect(rabbit.publishEvent).toHaveBeenCalledWith(expect.objectContaining({ type: "TaskStarted" }));
    });
    it("passage en CANCELLED → publie TaskCancelled", async () => {
        await request(app).put(`/tasks/${taskId}`).send({ status: "CANCELLED" });
        expect(rabbit.publishEvent).toHaveBeenCalledWith(expect.objectContaining({ type: "TaskCancelled" }));
    });
    it("passage en TODO → publie TaskReopened", async () => {
        ...
        expect(rabbit.publishEvent).toHaveBeenCalledWith(expect.objectContaining({ type: "TaskReopened" }));
    });
});
