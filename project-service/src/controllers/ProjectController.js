...
import { isNonEmptyString, badRequest, datesAreValid, getProjectById, parseBudgetOrDefault, parseDateOnlyOrNull, parsePositiveInt, parseProjectStatus, } from '../service/projectService.js';
export const createProjectController = (db) => {
    const createProject = async (req, res) => {
        try {
            ...
            if (!datesAreValid(startDate, dueDate)) {
                badRequest(req, res, 'Les dates sont invalides : la date de fin doit être supérieure ou égale à la date de début, et les dates doivent être superieur ou egale à la date actuelle.');
                return;
            }
            const budget = parseBudgetOrDefault(body.budget);
            const status = parseProjectStatus(body.status) ?? 'NOT_STARTED';
            if (ownerUserIdFromAuth &&
                ownerUserIdFromBody &&
            ...
            const [existingRows] = await db
                .promise()
                .query('SELECT id FROM projects WHERE owner_user_id = ? AND name = ?', [ownerUserId, name]);
            if (existingRows.length > 0) {
                res.status(409).json({
                    message: 'Un projet avec ce nom existe déjà pour cet utilisateur.',
                });
                return;
            }
            const [result] = await db
                .promise()
                .execute('INSERT INTO projects (owner_user_id, name, description, start_date, due_date, budget, status) VALUES (?, ?, ?, ?, ?, ?, ?)', [
                ownerUserId,
                name,
                description,
                startDate,
...
export const getProjectController = (db) => {
...
export const updateProjectController = (db) => {
    const updateProject = async (req, res) => {
        try {
            ...
            const dueDate = body.dueDate !== undefined
                ...
                : existing.dueDate;
            if (!datesAreValid(startDate, dueDate)) {
                badRequest(req, res, 'Les dates sont invalides : la date de fin doit être supérieure ou égale à la date de début, et les dates doivent être supérieures ou égales à la date actuelle.');
                return;
            }
            ...
            await db
                .promise()
                .execute('UPDATE projects SET name = ?, description = ?, start_date = ?, due_date = ?, budget = ?, status = ?, updated_at = NOW() WHERE id = ?', [name, description, startDate, dueDate, budget, status, id]);
...
export const deleteProjectController = (db) => {
    const deleteProject = async (req, res) => {
        try {
            ...
            const taskServiceUrlRaw = process.env.TASK_SERVICE_URL || 'http://localhost:3002';
            const taskServiceUrl = taskServiceUrlRaw.endsWith('/')
                ? taskServiceUrlRaw.slice(0, -1)
                : taskServiceUrlRaw;
            const response = await fetch(`${taskServiceUrl}/tasks?projectId=${id}`, {
            ...
            const body = (req.body ?? {});
            const forceDelete = req.query.force === 'true' || body.force === true;
...
export const getAllProjectsController = (db) => {
    const getAllProjects = async (req, res) => {
        try {
            ...
            const [rows] = await db
                .promise()
                .query('SELECT * FROM projects WHERE owner_user_id = ?', [userId]);
