...
describe("GET /auth/me", () => {
    ...
    it("token expiré → 401", async () => {
        const expiredToken = jwt.sign({ sub: "1", email: USER.email, exp: Math.floor(Date.now() / 1000) - 3600 }, process.env.JWT_SECRET);
    ...
    it("userId dans le token inexistant en base → 404", async () => {
        const fakeToken = jwt.sign({ sub: "99999", email: "fantome@test.com" }, process.env.JWT_SECRET);
