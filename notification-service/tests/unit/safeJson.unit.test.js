...
const makeMsg = (content) => ({ content: Buffer.from(content, "utf8") });
describe("safeJson", () => {
    ...
    it("retourne { raw } pour un JSON invalide", () => {
        const result = safeJson(makeMsg("pas du json"));
    ...
    it("retourne { raw } pour une string simple (non-JSON)", () => {
        const result = safeJson(makeMsg("Connexion WebSocket établie"));
